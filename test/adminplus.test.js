...
(async () => {
  ...
  try {
    ...
    A.setVcfg({ bonusPx: 50, bonusCr: 0 }); const f2 = A.find('Fama_U'); f2.day = { d: new Date().toISOString().slice(0, 10), px: 0 }; const before = f2.px; rf = A.awardMatch(f2, m); ok(rf.px === Math.round(BASE * 1.5) && f2.px - before === rf.px, 'con el bono al 50 %: ' + Math.round(BASE * 1.5) + ' PX (' + rf.px + ')');
    ...
    A.setVcfg({ bonusPx: 0, bonusCr: 0 }); const f3 = A.find('Fama_U'); f3.day = { d: new Date().toISOString().slice(0, 10), px: 0 }; ok(A.awardMatch(f3, m).px === BASE, 'con el bono a 0 cobra igual que los demás (' + BASE + ' PX)');
