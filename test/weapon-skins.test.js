...
(async () => {
  ...
  ok(before.length > 0 && before.filter(m => m.type === 'MeshToonMaterial').every(m => m.userData.rough == null && m.userData.metal == null && !m.map), 'sin skin, las piezas usan el toon básico: sin patrón ni acabado de skin');
  ok(before.some(m => m.type === 'MeshBasicMaterial' && m.side === w.THREE.BackSide), 'y el arma lleva su contorno negro (casco invertido)');
  ...
  const bodyMat1 = carbono.find(m => m.type === 'MeshToonMaterial' && m.map);
  ok(!!bodyMat1, 'con la skin Carbono, el cuerpo del arma lleva el material toon con la textura de patrón');
  ok(bodyMat1 && bodyMat1.userData.rough === sk1.rough && bodyMat1.userData.metal === sk1.metal && !!bodyMat1.gradientMap, 'y guarda la rugosidad/metal de la skin (' + sk1.rough + ' / ' + sk1.metal + '), que eligen la rampa de luz toon');
  ...
  ok(carbono.some(m => m.type === 'MeshToonMaterial' && !m.map && m.userData.rough == null), 'las piezas que NO son del color de la skin (miras, rieles) no reciben el patrón ni el acabado de la skin');
  ...
  ok(!after.some(m => m.map || m.userData && m.userData.rough != null), 'quitar la skin devuelve el arma al toon básico (ninguna pieza se queda con el patrón ni el acabado)');
