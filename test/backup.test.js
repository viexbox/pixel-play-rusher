...
(async () => {
  ...
  if (pgOk) {
    ...
    ok(pb.tables.user_avatars.rows.length === 1 && Buffer.from(pb.tables.user_avatars.rows[0].data.$b, 'base64').equals(q.P1) && pb.tables.market_sales.rows.length === 1 && pb.tables.schema_migrations.rows.length === 5, 'la foto (bytea) y las ventas van completas y las 5 migraciones constan');
