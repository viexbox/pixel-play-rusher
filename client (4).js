...
function addMesh(cx, y0, cz, w, h, d, color, solid) {
  ...
  worldUV(geo, w, h, d, TEX[type].tile, [uvR(), uvR()]); shadeBox(geo, 0.8, type === 'awning' ? WHITE : c);
...
const decoBox = (cx, y0, cz, w, h, d, color, basic) => {   // [NUEVO] va a un lote (una malla por material y sombra), no a una malla propia
  const geo = new THREE.BoxGeometry(w, h, d); shadeBox(geo, 1, colorOf(color)); geo.deleteAttribute('uv'); geo.translate(cx, y0 + h / 2, cz);
  queueBox(vcMat(basic ? 'basic' : 'lit'), !basic && h > 0.9, false, geo);
};
...
requestAnimationFrame(const vm = new KrunkerViewModel.ViewModel(THREE, { aspect: innerWidth / innerHeight });
vm.bindMouse(renderer.domElement);          // clic derecho = ADS, clic izquierdo = disparar
// cada fotograma:
vm.update(dt, { camera, velocity, onGround, sprinting });
renderer.render(scene, camera); vm.render(renderer);   // el arma se dibuja aparte, sin atravesar paredes);
})();
