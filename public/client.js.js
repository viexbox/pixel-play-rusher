...
const cfg = Object.assign({ name: '', cls: 0, map: 0, diff: 1, sens: 1, fov: 90, vol: 0.6, shadows: true, look: { col: 0, skin: 0 } }, store.get(K.cfg, {}));
cfg.look = { col: clamp((cfg.look && cfg.look.col) | 0, 0, 9), skin: clamp((cfg.look && cfg.look.skin) | 0, 0, 4) };
cfg.cls = clamp(cfg.cls | 0, 0, window.VoltShared.WEAPONS.length - 1); if (!cfg.optics || typeof cfg.optics !== 'object') cfg.optics = {}; cfg.map = clamp(cfg.map | 0, 0, 3); cfg.diff = clamp(cfg.diff | 0, 0, 2);
...
function newFighter(name, isPlayer, color) {
  return { name, isPlayer, color, pos: new THREE.Vector3(), vel: new THREE.Vector3(), yaw: 0, pitch: 0, hp: 100, alive: false,
...
function makeLabel(text) {
  ...
  g.fillStyle = 'rgba(8,11,24,0.86)'; g.fillRect(0, 8, 256, 48); g.fillStyle = '#ffdc3a'; g.fillRect(0, 8, 6, 48);
  g.fillStyle = '#fff'; g.font = '800 30px "Exo 2", "Barlow Semi Condensed", Arial, sans-serif'; g.textAlign = 'center'; g.textBaseline = 'middle';
  g.fillText(text, 128, 33, 240);
...
function killPopup(name, pts, head, streak) {
  ...
  el.killcard.innerHTML = '<div class="l1">ELIMINASTE A <b>' + esc(name) + '</b><span class="pts">+' + pts + '</span></div><div class="l2">' + tags.join(' · ') + '</div>';
  el.killcard.classList.remove('on'); void el.killcard.offsetWidth; el.killcard.classList.add('on');
}
...
function feedAdd(k, v, weapon, head) {
  const li = document.createElement('li'); li.className = (k.isPlayer ? 'mine' : '') + (v.isPlayer ? ' dead' : '');
  li.innerHTML = '<span class="n' + (k.isPlayer ? ' me' : '') + '">' + esc(k.isPlayer ? 'TÚ' : k.name) + '</span>' + (WICON[weapon] || '') + (head ? HEAD_ICON : '') + '<span class="n' + (v.isPlayer ? ' me' : '') + '">' + esc(v.isPlayer ? 'TÚ' : v.name) + '</span>';
...
function tableRows(list, meRef) {
  return list.map((f, i) => '<tr class="' + (f === meRef ? 'me ' : '') + (i === 0 ? 'first' : '') + '"><td class="pos">' + (i + 1) + '</td><td>' + esc(f.name) + '</td><td class="r">' + f.kills + '</td><td class="r">' + f.deaths + '</td><td class="r">' + f.points + '</td></tr>').join('');
}
let hudAcc = 0;
function updateHudSlow() {
  const s = sortedFighters(), me = player, mi = s.indexOf(me);
  const rows = s.slice(0, 5); if (mi >= 5) rows.push(me);
  el.live.innerHTML = rows.map(f => '<li class="' + (f.isPlayer ? 'me' : '') + '"><span>' + (s.indexOf(f) + 1) + '</span><span class="nm">' + esc(f.name) + '</span><i>' + f.kills + '</i><b>' + f.points + '</b></li>').join('');
  el.lbPlace.textContent = (mi + 1) + '/' + s.length;
  setTxt(el.myPts, 'pts', me.points); setTxt(el.myKD, 'kd', me.kills + ' K · ' + me.deaths + ' M');
  const lead = s[0]; setTxt(el.leadName, 'ln', lead.isPlayer ? 'TÚ' : lead.name); setTxt(el.leadPts, 'lp', lead.points + ' pts');
  ...
  setTxt(hs.name, 'hsn', cfg.name); setTxt(hs.lvl, 'hsl', 'NV ' + L.lvl);
...
function playerShoot() {
  ...
  for (let i = 0; i < w.pellets; i++) {
    ...
    if (i < 3 || w.pellets === 1) tracer(muzzle, r.point, '#fff1b8');
  }
...
function startOnline() {
  ...
  ws.onopen = () => netSend({ t: 'hello', v: 1, n: cfg.name, map: cfg.map, c: cfg.cls, lk: [cfg.look.col, cfg.look.skin] });
...
function setPauseTexts(kind) {
  ...
  $('#quit').textContent = on ? 'Salir al menú' : 'Abandonar';
}
...
function netHandle(m) {
  switch (m.t) {
    ...
    case 'chat': return chatAdd(m.id === net.id ? 'me' : '', m.n, m.m);
...
function onWelcome(m) {
  ...
  clearFighters();
  player = newFighter(cfg.name, true, '#ffc857');
...
function addRemote(p) {
  ...
  f.seed = p.id; f.wi = p.c || 0; f.mesh = buildBot(f.color, f.wi, f.seed, f.skin); f.label = makeLabel(f.name);
...
function onNetKill(m) {
  ...
  if (k === player) {
    ...
    sfx.kill(); hitmark('kill');
    killPopup(v.name, m.pts, !!m.h, m.streak);
  }
  if (v === player) {
...
function onNetEnd(m) {
  ...
  $('#endRows').innerHTML = rows.map((r, i) => '<tr class="' + (r[0] === net.id ? 'me ' : '') + (i === 0 ? 'first' : '') + '"><td class="pos">' + (i + 1) + '</td><td>' + esc(r[1]) + '</td><td class="r">' + r[2] + '</td><td class="r">' + r[3] + '</td><td class="r">' + r[4] + '</td></tr>').join('');
...
const TABS = ['maps', 'rank', 'controls', 'settings'];
...
function renderMenuStats() {
  ...
  $('#lvlN').textContent = 'NIVEL ' + L.lvl; $('#lvlBar').style.width = L.frac * 100 + '%'; $('#lvlTxt').textContent = pts + ' / ' + L.next;
  renderKr();
}
...
let lobbyWs = null;
function chatAdd(kind, name, text) {
  const li = document.createElement('li'); li.className = kind;
  li.innerHTML = (name ? '<b>' + esc(name) + '</b>' : '') + esc(text);
...
function lobbyConnect() {
  ...
  lobbyWs = ws;
  ws.onopen = () => { ws.send(JSON.stringify({ t: 'lobby', n: cfg.name })); updateChatCh(); };
  ws.onmessage = ev => {
    let m; try { m = JSON.parse(ev.data); } catch (e) { return; }
    if (m.t === 'chat') chatAdd(m.n === cfg.name ? 'me' : '', m.n, m.m);
