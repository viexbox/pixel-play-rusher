...
/* [NUEVO] Dar / quitar / fijar PX o Créditos de una cuenta por su nombre */
async function coinOp(name, op, cur, amount, reason) {
  const n = Math.abs(Math.trunc(+amount)), lbl = cur === 'cr' ? 'Créditos' : 'PX';
  if (!(n >= 0) || (op !== 'set' && !n)) { toast('Escribe una cantidad mayor que 0', true); return null; }
  const r = op === 'set' ? await act(() => api('POST', '/coins/set', { username: name, currency: cur, value: n, reason }), null) : await act(() => api('POST', cur === 'cr' ? '/credits' : '/px', { username: name, delta: (op === 'sub' ? -1 : 1) * n, reason }), null);
  if (!r) return null; const bal = r.balance != null ? r.balance : r.credits;
  toast(r.applied === 0 ? 'Sin cambios en ' + name + ' (' + bal + ' ' + lbl + ')' : name + ': ' + (r.applied > 0 ? '+' : '') + r.applied + ' ' + lbl + ' → ahora ' + bal + (op === 'sub' && r.applied > -n ? ' (tenía menos de lo pedido)' : '')); return r;
}
const views = {
  ...
  /* ===== [NUEVO] Verificados: se escribe el nombre de la cuenta y ya tiene el tic y los beneficios (sin códigos) ===== */
  async influencers() {
    const [v, inf, ac] = await Promise.all([api('GET', '/verified'), api('GET', '/influencers'), api('GET', '/accounts?q=')]);
    const c = v.cfg;
    const benefits = ['tic azul en el chat, el marcador, la clasificación y el perfil', 'efecto dorado en sus bajas', c.chat ? 'puede escribir en el chat aunque esté bloqueado por la moderación' : null, c.bonusPx || c.bonusCr ? '+' + c.bonusPx + ' % de PX y +' + c.bonusCr + ' % de Créditos en cada partida (los topes diarios siguen valiendo)' : null, c.giftPx || c.giftCr ? 'regalo al verificar: ' + (c.giftPx ? c.giftPx + ' PX ' : '') + (c.giftCr ? c.giftCr + ' Créditos' : '') : null].filter(Boolean);
    const verify = async (name, note) => {
      name = String(name || '').trim(); if (!name) { toast('Escribe el nombre de la cuenta', true); return null; }
      const r = await act(() => api('POST', '/verified/set', { username: name, note: note || '' }), null);
      if (r) { toast(r.already ? r.username + ' ya estaba verificado' : '✔ ' + r.username + ' ya está verificado y tiene sus beneficios'); render(true); } return r;
    };
    const names = h('datalist', { id: 'vNames' }, ac.accounts.map(a => h('option', { value: a.username })));
    let tm = null; const search = ev => { clearTimeout(tm); tm = setTimeout(async () => { try { const r = await api('GET', '/accounts?q=' + encodeURIComponent(ev.target.value)); names.replaceChildren(...r.accounts.map(a => h('option', { value: a.username }))); } catch (e) { /* sin sugerencias */ } }, 250); };
    const rows = v.accounts.map(a => h('tr', null, h('td', null, nameEl(a.username, 'inf')), h('td', { class: 'muted', text: a.note || '' }), h('td', { text: a.since ? fmtD(a.since) : '' }), h('td', { class: 'muted', text: a.by || '' }), h('td', { text: a.px + ' PX · ' + a.credits + ' CR' }),
      h('td', null, h('button', { class: 'btn sm', text: 'Nota', onclick: async () => { const x = await ask('Nota de ' + a.username, [{ name: 'note', label: 'Nota (canal, motivo…)', value: a.note || '' }]); if (x) { await act(() => api('POST', '/verified/set', { username: a.username, note: x.note }), 'Nota guardada'); render(true); } } }), ' ',
        h('button', { class: 'btn sm red', text: 'Quitar verificado', onclick: async () => { if (!window.confirm('¿Quitar el verificado a ' + a.username + '? Perderá el tic y los beneficios.')) return; await act(() => api('POST', '/verified/set', { username: a.username, verified: false }), 'Verificado retirado'); render(true); } }))));
    const num = (id, label, val, max) => h('label', { style: 'display:flex;flex-direction:column;gap:4px;font-size:13px' }, label, h('input', { id, type: 'number', min: 0, max, value: val, style: 'width:120px' }));
    const legacy = inf.influencers.map(i => h('tr', null, h('td', null, nameEl(i.name, 'inf')), h('td', { class: 'muted', text: i.note || '' }), h('td', null, h('span', { class: 'tag ' + (i.active ? 'ok' : 'red'), text: i.active ? 'Clave activa' : 'Revocada' })),
      h('td', null, i.active ? h('button', { class: 'btn sm gold', text: 'Pasar a verificación por cuenta', onclick: async () => { const r = await act(() => api('POST', '/verified/set', { username: i.name, note: i.note || 'antes influencer con clave' }), null); if (r) { await act(() => api('POST', '/influencers/revoke', { id: i.id }), '✔ ' + r.username + ' verificado por cuenta; su clave antigua ya no hace falta'); render(true); } } }) : null, ' ', i.active ? h('button', { class: 'btn sm red', text: 'Revocar clave', onclick: async () => { if (!window.confirm('¿Revocar la clave de ' + i.name + '?')) return; await act(() => api('POST', '/influencers/revoke', { id: i.id }), 'Clave revocada'); render(true); } }) : null)));
    return h('div', null, h('h2', { text: 'Verificados ✔' }),
      h('p', { class: 'muted', text: 'Escribe el nombre de la cuenta y pulsa «Verificar ahora»: al instante tiene el tic azul y todos los beneficios. No hay ningún código que enviar ni pegar. La persona tiene que haberse registrado antes en el juego.' }),
      h('div', { class: 'tools', style: 'margin:12px 0;display:flex;gap:10px;flex-wrap:wrap;align-items:center' }, h('input', { id: 'vName', list: 'vNames', placeholder: 'Nombre de la cuenta (ej. ProGamer)', maxlength: 20, autocomplete: 'off', oninput: search }), names, h('input', { id: 'vNote', placeholder: 'Nota (canal, motivo…)', maxlength: 80 }), h('button', { class: 'btn gold', text: '✔ Verificar ahora', onclick: async () => { const r = await verify($('#vName').value, $('#vNote').value); if (r) { $('#vName').value = ''; $('#vNote').value = ''; } } })),
      h('div', { class: 'card', style: 'padding:12px 16px;margin:6px 0 16px;text-align:left' }, h('b', { text: 'Qué recibe una cuenta verificada' }), h('ul', { style: 'margin:6px 0 0 18px' }, benefits.map(t => h('li', { text: t })))),
      h('h3', { text: 'Cuentas verificadas (' + v.accounts.length + ')' }), v.accounts.length ? table(['Cuenta', 'Nota', 'Desde', 'Por', 'Saldo', ''], rows) : h('p', { class: 'muted', text: 'Todavía no has verificado a nadie.' }),
      h('h3', { text: 'Beneficios de los verificados' }), h('p', { class: 'muted', text: 'Puedes cambiarlos cuando quieras; se aplican a todos los verificados a la vez. El tic azul y el efecto dorado siempre están.' }),
      h('div', { class: 'tools', style: 'display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end' },
        h('label', { style: 'display:flex;flex-direction:column;gap:4px;font-size:13px' }, 'Chat con la moderación activada', h('select', { id: 'vChat' }, h('option', { value: '1', text: 'Pueden escribir', selected: c.chat }), h('option', { value: '0', text: 'Como los demás', selected: !c.chat }))),
        num('vBpx', 'Bono de PX por partida (%)', c.bonusPx, 200), num('vBcr', 'Bono de Créditos por partida (%)', c.bonusCr, 200), num('vGpx', 'Regalo de PX al verificar', c.giftPx, 100000), num('vGcr', 'Regalo de Créditos al verificar', c.giftCr, 100000),
        h('button', { class: 'btn', text: 'Guardar beneficios', onclick: async () => { await act(() => api('POST', '/verified/benefits', { chat: $('#vChat').value === '1', bonusPx: +$('#vBpx').value, bonusCr: +$('#vBcr').value, giftPx: +$('#vGpx').value, giftCr: +$('#vGcr').value }), 'Beneficios guardados'); render(true); } })),
      h('h3', { text: 'Claves de influencer antiguas' }), h('p', { class: 'muted', text: 'Ya no hacen falta. Si alguien todavía usa su clave sigue funcionando; pásalo a verificación por cuenta y su clave queda revocada.' }), inf.influencers.length ? table(['Nombre', 'Nota', 'Estado', ''], legacy) : h('p', { class: 'muted', text: 'No hay claves antiguas.' }));
  },
  /* ===== [NUEVO] Cuentas: ver y gestionar ===== */
  async accounts() {
    const q = $('#accQ') ? $('#accQ').value : '', ac = await api('GET', '/accounts?q=' + encodeURIComponent(q));
    const coins = async a => {
      const x = await ask('Monedas de ' + a.username + ' (' + a.px + ' PX · ' + a.credits + ' CR)', [{ name: 'op', label: 'Qué hacer', type: 'select', options: [['add', 'Dar'], ['sub', 'Quitar'], ['set', 'Fijar saldo exacto']] }, { name: 'cur', label: 'Moneda', type: 'select', options: [['px', 'PX (moneda de pago)'], ['cr', 'Créditos (se ganan jugando)']] }, { name: 'amount', label: 'Cantidad', type: 'number', required: true, value: '100' }, { name: 'reason', label: 'Motivo (queda en la auditoría)', maxlength: 80 }], 'Aplicar');
      if (x) { await coinOp(a.username, x.op, x.cur, x.amount, x.reason); render(true); }
    };
    const more = async a => {
      const x = await ask('Más sobre ' + a.username, [{ name: 'a', label: 'Acción', type: 'select', options: [['code', 'Generar código de recuperación (para quien perdió el acceso)'], ['mail', a.emailOk ? 'Quitar la marca de correo verificado' : 'Marcar el correo como verificado'], a.deleteAt ? ['undel', 'Cancelar la eliminación programada'] : ['sched', 'Programar la eliminación (con plazo para arrepentirse)'], ['now', 'ELIMINAR AHORA (sin plazo, no se puede deshacer)']] }], 'Continuar');
      if (!x) return;
      if (x.a === 'code') { const r = await act(() => api('POST', '/accounts/reset-code', { username: a.username }), null); if (r) await ask('Código para ' + a.username, [{ name: 'c', label: 'Dáselo al jugador (vale ' + r.minutes + ' min). Debe pulsar «¿Has olvidado tu contraseña?», escribir su correo, este código y la contraseña nueva.', value: r.code }], 'Cerrar'); }
      else if (x.a === 'mail') { await act(() => api('POST', '/accounts/verify-email', { username: a.username, verified: !a.emailOk }), 'Hecho'); render(true); }
      else if (x.a === 'sched') { await act(() => api('POST', '/accounts/delete', { username: a.username, mode: 'schedule' }), 'Eliminación programada'); render(true); }
      else if (x.a === 'undel') { await act(() => api('POST', '/accounts/delete', { username: a.username, mode: 'cancel' }), 'Eliminación cancelada'); render(true); }
      else if (x.a === 'now') { const y = await ask('Eliminar ' + a.username + ' AHORA', [{ name: 'confirm', label: 'Se borra TODO (cuenta, objetos, foto, amigos, pase). Escribe su nombre exacto para confirmar.' }], 'Eliminar para siempre'); if (y) { await act(() => api('POST', '/accounts/delete', { username: a.username, mode: 'now', confirm: y.confirm }), 'Cuenta eliminada'); render(true); } }
    };
    const rows = ac.accounts.map(a => h('tr', null, h('td', null, nameEl(a.username, a.verified ? 'inf' : 0)), h('td', null, h('span', { class: 'muted', text: a.email2 + ' ' }), h('span', { class: 'tag ' + (a.emailOk ? 'ok' : 'gold'), text: a.emailOk ? 'verificado' : 'sin verificar' })), h('td', { text: a.px }), h('td', { text: a.credits }), h('td', { text: a.points }), h('td', { text: fmtD(a.createdAt) }), h('td', { text: a.lastLogin ? fmtD(a.lastLogin) : '' }),
      h('td', null, a.deleteAt ? h('span', { class: 'tag red', text: 'se elimina el ' + new Date(a.deleteAt).toLocaleDateString('es-ES') }) : null),
      h('td', null, h('button', { class: 'btn sm', text: 'Monedas', onclick: () => coins(a) }), ' ', h('button', { class: 'btn sm ' + (a.verified ? 'red' : 'gold'), text: a.verified ? 'Quitar ✔' : '✔ Verificar', onclick: async () => { await act(() => api('POST', '/verified/set', { username: a.username, verified: !a.verified }), a.verified ? 'Verificado retirado' : '✔ ' + a.username + ' verificado'); render(true); } }), ' ', h('button', { class: 'btn sm', text: 'Más…', onclick: () => more(a) }))));
    return h('div', null, h('h2', { text: 'Cuentas (' + ac.total + ')' }), h('p', { class: 'muted', text: 'Busca por nombre o correo. Desde aquí puedes dar o quitar monedas, verificar, ayudar a quien perdió el acceso o eliminar una cuenta. Todo queda en la auditoría.' }),
      h('div', { class: 'tools', style: 'margin:14px 0;display:flex;gap:10px;align-items:center' }, h('input', { id: 'accQ', placeholder: 'Buscar por usuario o correo', value: q, onkeydown: ev => { if (ev.key === 'Enter') render(true); } }), h('button', { class: 'btn', text: 'Buscar', onclick: () => render(true) })),
      table(['Usuario', 'Correo', 'PX', 'CR', 'Puntos', 'Creada', 'Último acceso', 'Estado', ''], rows), ac.accounts.length ? null : h('p', { class: 'muted', text: 'No hay cuentas que coincidan.' }));
  },
  /* ===== [NUEVO] Monedas y ventas (dinero real) ===== */
  async economy() {
    const [sa, lg, ac] = await Promise.all([api('GET', '/sales'), api('GET', '/coins/log'), api('GET', '/accounts?q=')]);
    const money = (cents, cur) => (cents / 100).toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + String(cur || sa.currency).toUpperCase();
    const rev = o => { const e = Object.entries(o || {}); return e.length ? e.map(([c, v]) => money(v, c)).join(' + ') : money(0, sa.currency); };
    const chart = () => { const d = sa.byDay, max = Math.max(1, ...d.map(x => x.amount)), W = 600, H = 84, bw = W / d.length; const el = h('div', { style: 'margin:8px 0 14px' }); el.innerHTML = '<svg viewBox="0 0 ' + W + ' ' + (H + 18) + '" width="100%" style="max-width:640px" role="img" aria-label="Ingresos de los últimos 30 días">' + d.map((x, i) => { const hh = Math.round(x.amount / max * H); return '<rect x="' + (i * bw + 1) + '" y="' + (H - hh) + '" width="' + (bw - 2) + '" height="' + Math.max(hh, x.amount ? 2 : 1) + '" fill="' + (x.amount ? '#ffdc3a' : '#2a3358') + '"><title>' + x.d + ': ' + money(x.amount) + ' (' + x.n + ' pedidos)</title></rect>'; }).join('') + '<text x="0" y="' + (H + 14) + '" fill="#8a94c0" font-size="10">hace 30 días</text><text x="' + W + '" y="' + (H + 14) + '" fill="#8a94c0" font-size="10" text-anchor="end">hoy</text></svg>'; return el; };
    const names = h('datalist', { id: 'cNames' }, ac.accounts.map(a => h('option', { value: a.username })));
    let tm = null; const search = ev => { clearTimeout(tm); tm = setTimeout(async () => { try { const r = await api('GET', '/accounts?q=' + encodeURIComponent(ev.target.value)); names.replaceChildren(...r.accounts.map(a => h('option', { value: a.username }))); } catch (e) { /* sin sugerencias */ } }, 250); };
    const go = async op => { const n = $('#cName').value.trim(); if (!n) { toast('Escribe el nombre de la cuenta', true); return; } const ok = await coinOp(n, op, $('#cCur').value, $('#cAmt').value, $('#cWhy').value); if (ok) { $('#cAmt').value = '100'; render(true); } };
    const orders = sa.orders_list.map(o => h('tr', null, h('td', { text: fmtD(o.ts) }), h('td', { text: o.user }), h('td', { text: o.px + ' PX' }), h('td', { text: money(o.amount, o.currency) }), h('td', null, h('span', { class: 'tag ' + (o.status === 'paid' || o.status === 'manual' ? 'ok' : o.status === 'pending' ? 'gold' : 'red'), text: { paid: 'Pagado', manual: 'Venta manual', pending: 'Pendiente', refunded: 'Reembolsado', orphan: 'Sin cuenta' }[o.status] || o.status })), h('td', { class: 'muted', text: o.note || (o.pack === 'manual' ? '' : o.pack) }),
      h('td', null, o.status === 'paid' || o.status === 'manual' ? h('button', { class: 'btn sm red', text: 'Reembolsar', onclick: async () => { if (!window.confirm('¿Marcar como reembolsado el pedido de ' + o.user + ' (' + money(o.amount, o.currency) + ') y retirarle los ' + o.px + ' PX que aún conserve?')) return; const r = await act(() => api('POST', '/orders/refund', { id: o.id }), null); if (r) { toast('Reembolsado: −' + r.removed + ' PX. ' + r.note); render(true); } } }) : null)));
    const exportCsv = () => { const q = s => '"' + String(s == null ? '' : s).replace(/"/g, '""') + '"', lines = [['fecha', 'cuenta', 'px', 'importe', 'moneda', 'estado', 'pedido', 'nota'].join(',')].concat(sa.orders_list.map(o => [q(new Date(o.ts).toISOString()), q(o.user), o.px, ((o.amount || 0) / 100).toFixed(2), q(o.currency), q(o.status), q(o.id), q(o.note || '')].join(','))), a = h('a', { href: URL.createObjectURL(new Blob(['\ufeff' + lines.join('\n')], { type: 'text/csv;charset=utf-8' })), download: 'pedidos-' + new Date().toISOString().slice(0, 10) + '.csv' }); document.body.append(a); a.click(); setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 1000); };
    const logs = lg.log.map(l => h('tr', null, h('td', { text: fmtD(l.ts) }), h('td', { text: l.user }), h('td', { text: l.cur }), h('td', { class: l.delta >= 0 ? '' : 'muted', text: (l.delta >= 0 ? '+' : '') + l.delta }), h('td', { text: l.balance }), h('td', { class: 'muted', text: l.reason }), h('td', { class: 'muted', text: l.by })));
    const inp = (id, ph, extra) => h('input', Object.assign({ id, placeholder: ph }, extra || {}));
    return h('div', null, h('h2', { text: 'Monedas y ventas' }),
      h('div', { class: 'cards' }, card('Ingresos hoy', rev(sa.revenue.today)), card('Últimos 7 días', rev(sa.revenue.d7)), card('Últimos 30 días', rev(sa.revenue.d30)), card('Total', rev(sa.revenue.all)), card('Pedidos pagados', sa.orders.paid + sa.orders.manual), card('PX vendidos', sa.pxSold), card('Compra media', money(sa.average)), card('PX en circulación', sa.circulation.px), card('Créditos en circulación', sa.circulation.cr)),
      h('p', { class: 'muted', text: (sa.storeOn ? 'La tienda de pago (Stripe) está activada. ' : 'La tienda de pago NO está activada: solo verás ventas manuales. ') + 'Los ingresos cuentan pedidos pagados y ventas manuales, sin los reembolsados (' + sa.orders.refunded + ') ni los pendientes (' + sa.orders.pending + ').' }), chart(),
      h('h3', { text: 'Dar o quitar monedas' }), h('p', { class: 'muted', text: 'Escribe el nombre de la cuenta. PX es la moneda que se compra con dinero real; los Créditos solo se ganan jugando. Nadie se queda con saldo negativo y todo queda en la auditoría.' }),
      h('div', { class: 'tools', style: 'display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:8px 0' }, inp('cName', 'Nombre de la cuenta', { list: 'cNames', maxlength: 20, autocomplete: 'off', oninput: search }), names, h('select', { id: 'cCur' }, h('option', { value: 'px', text: 'PX (dinero real)' }), h('option', { value: 'cr', text: 'Créditos' })), inp('cAmt', 'Cantidad', { type: 'number', min: 0, value: '100', style: 'width:110px' }), inp('cWhy', 'Motivo', { maxlength: 80 }),
        h('button', { class: 'btn gold', text: '＋ Dar', onclick: () => go('add') }), h('button', { class: 'btn red', text: '− Quitar', onclick: () => go('sub') }), h('button', { class: 'btn', text: '= Fijar saldo', onclick: () => go('set') })),
      h('h3', { text: 'Registrar una venta con dinero real' }), h('p', { class: 'muted', text: 'Para cuando alguien te paga fuera de la tienda (Bizum, PayPal, transferencia…): entrega los PX y la venta cuenta en los ingresos.' }),
      h('div', { class: 'tools', style: 'display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:8px 0' }, inp('mName', 'Nombre de la cuenta', { list: 'cNames', maxlength: 20 }), inp('mPx', 'PX a entregar', { type: 'number', min: 1, style: 'width:120px' }), inp('mAmt', 'Importe pagado (' + String(sa.currency).toUpperCase() + ')', { type: 'number', min: 0, step: '0.01', style: 'width:150px' }), inp('mNote', 'Nota (Bizum, PayPal…)', { maxlength: 60 }),
        h('button', { class: 'btn gold', text: 'Registrar venta y entregar PX', onclick: async () => { const r = await act(() => api('POST', '/orders/manual', { username: $('#mName').value, px: +$('#mPx').value, amount: $('#mAmt').value || 0, note: $('#mNote').value, currency: sa.currency }), null); if (r) { toast('Venta registrada: ' + r.order.user + ' recibe ' + r.order.px + ' PX'); render(true); } } })),
      sa.top.length ? h('div', null, h('h3', { text: 'Mejores compradores' }), table(['Cuenta', 'Gastado', 'PX', 'Compras'], sa.top.map(t => h('tr', null, h('td', { text: t.user }), h('td', { text: money(t.amount) }), h('td', { text: t.px }), h('td', { text: t.n }))))) : null,
      h('h3', { text: 'Pedidos' }), h('div', { style: 'margin:6px 0' }, h('button', { class: 'btn sm', text: 'Descargar CSV', onclick: exportCsv })), sa.orders_list.length ? table(['Fecha', 'Cuenta', 'PX', 'Importe', 'Estado', 'Detalle', ''], orders) : h('p', { class: 'muted', text: 'Todavía no hay pedidos.' }),
      h('h3', { text: 'Movimientos de monedas' }), lg.log.length ? table(['Fecha', 'Cuenta', 'Moneda', 'Cambio', 'Saldo', 'Motivo', 'Por'], logs) : h('p', { class: 'muted', text: 'Todavía no hay movimientos.' }));
  },
  /* ===== [NUEVO] Antes de lanzar ===== */
  async launch() {
    const d = await api('GET', '/launch'), L = { must: 'IMPRESCINDIBLE', should: 'RECOMENDADO', info: 'INFORMACIÓN' };
    return h('div', null, h('h2', { text: 'Antes de lanzar' }),
      h('div', { class: d.ready ? 'card' : 'err', style: 'padding:12px 16px;margin:8px 0 16px;border:2px solid ' + (d.ready ? '#37f29a' : '#ff4d5a') + ';border-radius:10px' }, h('b', { text: d.ready ? '✔ Lo imprescindible está listo.' : '✖ Falta algo imprescindible antes de abrir el juego al público.' })),
      d.items.map(i => h('div', { style: 'display:flex;gap:12px;align-items:flex-start;padding:10px 0;border-top:1px solid rgba(255,255,255,.1)' }, h('b', { style: 'font-size:20px;width:26px;color:' + (i.ok ? '#37f29a' : i.level === 'must' ? '#ff4d5a' : i.level === 'should' ? '#ffb020' : '#8a94c0'), text: i.ok ? '✔' : i.level === 'info' ? 'ℹ' : '✖' }), h('div', null, h('div', null, h('b', { text: i.title + ' ' }), h('span', { class: 'tag', text: L[i.level] })), h('div', { class: i.ok ? 'muted' : '', text: i.detail })))));
  },
  async maint() {
