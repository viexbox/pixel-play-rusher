'use strict';
/* Krunxa · Seguridad de las cuentas: correo vinculado, recuperación y eliminación.
...
function createAccountSecurity(ctx) {
  ...
  async function mail(to, subject, text) { try { return await admin.sendMail(to, subject, text + '\n\n—\nKrunxa. Si no has sido tú, puedes ignorar este correo.'); } catch (e) { log('No se pudo enviar el correo a ' + mask(to) + ': ' + e.message); return false; } }
