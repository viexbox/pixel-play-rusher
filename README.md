...
**Todavía no están conectados a la partida.** El arma en primera persona se sigue dibujando con cajas por código (`gunModel()` en `client.js`), como siempre. Cambiar eso para que cargue estos modelos es una decisión de arquitectura aparte (afecta a la animación de retroceso, el ADS, el balanceo del arma) que no se ha tomado todavía. Los `.glb` están listos para esa integración cuando se decida.
