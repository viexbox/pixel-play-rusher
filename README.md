...
## Modelos 3D de armas y sistema de skins ampliado

**Modelos:** `scripts/generate_weapons.py` genera, con Blender de verdad (`blender --background --python scripts/generate_weapons.py`), 13 modelos low-poly con las partes separadas y con nombre (`body`, `magazine`, `barrel`, `sight_scope`...), exportados a `.glb` en `public/models/weapons/`: las 11 armas del juego más un lanzacohetes y una ballesta (modelos nuevos, sin mecánica de disparo — el juego solo tiene armas de trayectoria instantánea). `public/vendor/GLTFLoader.js` (Three.js r128, la misma versión que ya usa el juego) puede cargarlos; verificado cargándolos de verdad con el motor del juego, no solo comprobando que el archivo existe.

**Ya conectados a la partida: 9 de las 11 armas.** Asalto, Ráfaga, Torrente, Trueno, Sheriff, Precisión, Dúo, Vórtice y Centinela usan de verdad su modelo `.glb` en primera persona: `preloadGunModels()` los precarga al arrancar (`GUN_GLTF` en `client.js`), y `gunModel()` los usa en cuanto están listos, quitando las miras propias del modelo (las pone el sistema de miras intercambiables de siempre, sin tocarlo) y pintando cada pieza con el color de la skin puesta (`gunPartColor()` + `gunMat()`). Mientras un modelo no ha cargado (o si falla), el arma se sigue viendo con cajas — nunca queda invisible ni rompe la partida.

**AK y Lince siguen con cajas, a propósito.** Tienen geometría propia hecha a mano (madera y mira telescópica con zoom) que no se ha tocado esta sesión — más delicado de cambiar sin más tiempo de comprobación. `public/models/weapons/ak.glb` y `lince.glb` existen (generados igual que los demás) pero no se cargan en la partida.

**Sistema de skins:** se amplió `WEAPON_SKINS` (`public/shared.js`, ya existía de una sesión anterior) con `rough`/`metal` (acabado), `glow` (brillo emisivo) y `pattern` (nombre de un patrón nuevo en la tabla `TEX` de `client.js`: `carbono`, `camuflaje`, `rayas`). Centinela y Vórtice, que antes no tenían ninguna skin, ya tienen 3 cada una. Todo aplicado de verdad al arma en primera persona vía `gunMat()`: verificado con capturas del juego real, no solo con los datos. Prueba: `test/weapon-skins.test.js`.
